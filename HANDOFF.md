...
⚠️ **`GUIDE_PRO.md` 는 아직 옛 내용입니다** — `parser.js` 를 "정규식 파서" 로 설명하는 곳이
여럿 있는데, 그건 `91f1a13` 에서 **AST 로 전면 재작성되기 전** 이야기입니다.
잘린 함수를 2줄로 보고한다는 설명(346줄 근처)도 지금은 맞지 않습니다. **손보려면 통째로 볼 것.**
