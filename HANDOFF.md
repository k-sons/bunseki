...
1. `git log --oneline -5` — 맨 위가 **인계문 갱신**, 그 아래가 `e6496ad`(C-3) 인지 확인.
2. `npm test` — **123 pass / 0 fail** 이면 출발점이 맞음.
...
- ✅ **작업트리 깨끗. 미커밋 없음.** `main` 최신 = `e6496ad`
  (A 트랙 + B + E + C + D + D-2 + C-2 + F + **C-3** 완료).
- ✅ `npm test` → **123 pass / 0 fail**
  (parser 10 · behavior 18 · examples 3 · timing 43 · stale-deps 12 · interplay 19 ·
  hook-boundary 18)
...
**F) `.catch`/`.finally`** · **C-3) 관문의 사각지대** 까지 완료·커밋됨.

### ▶ 다음: **아직 정하지 않음 — 먼저 합의할 것**

지금까지의 흐름(⚡ 동작 탭을 "무슨 일이 일어나나" 로 정확하게 만드는 것)에서
이어질 만한 후보를 적어 둡니다. **고르기 전에 착수하지 말 것.**

**(가) G) 알약 두 개 정리 — 작음**
`if (x) { setY(); return }` 이 관문 알약 + setter 의 `조건부` 꼬리표로 **두 번** 보입니다.
둘이 말하는 건 다르지만(어디서 멈추나 / 이 setter 는 그 가지에서만 불리나) 한 조건에서
알약이 둘 서는 게 시끄러우면 정리. **실제 화면에서 시끄러운지 먼저 볼 것** — 아니면 건드리지 말 것.

**(나) 정리(cleanup) 함수가 하는 일 — 중간**
지금 `정리(cleanup) 실행` 은 **있다/없다** 뿐입니다. 무엇을 되돌리는지
(`clearInterval` · `abort()` · `alive = false` · 구독 해제)를 읽어 주면
"이 effect 가 나갈 때 무엇이 멈추나" 가 보입니다. 새 판정이 아니라 **읽어서 적기**.

**(다) 이벤트 핸들러 안의 비동기 — 중간**
⏱ 섹션은 **Effect 만** 봅니다. `onClick={async () => { … await … setX() }}` 도
같은 언마운트 위험·같은 대기 구간이 있는데 지금은 연쇄에 `호출` 한 줄로만 보입니다.
**판정 규칙은 이미 다 있음**(`analyzeBody`) — 대상을 넓히는 일.

**(라) 실제형 파일로 다시 다지기 — 작음~중간**
E) 이후 C·D·D-2·C-2·F·C-3 이 얹혔습니다. 큰 파일·좁은 화면으로 다시 한 번 훑어
오탐/깨짐/성능을 확인. **넓힌 판정(C-3)이 헛경보를 만들지 않는지**가 특히 볼 곳.

**추천**: (라) → (다). C-3 이 판정을 넓힌 직후라 **다지기가 먼저** 오는 게 안전하고,
그다음이 "아직 안 보이는 큰 덩어리" 인 (다) 입니다.
