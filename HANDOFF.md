...
1. `git log --oneline -5` — 맨 위가 **인계문 갱신**, 그 아래가 `3355595`(F) 인지 확인.
...
> **다음 세션은 `C-3` 부터 시작하기로 정해져 있습니다** — 6장을 그대로 따라가면 됩니다.
> 합의는 이미 끝났으니 다시 묻지 말 것.
...
### ▶ 다음: C-3) 관문의 사각지대 — **여기부터 시작 (합의됨)**

**무엇이 문제인가** — 이른 반환의 블록에 문장이 **둘 이상**이면 지금은 못 봅니다:

```js
useEffect(() => {
  if (!id) { reset(); return }     // ← 관문으로 안 세고, 뒤의 setter 도 가드로 안 봄
  fetchPosts(id).then(setPosts)
}, [id])
```

`isEarlyReturnGuard()` 가 `c.body.length === 1` 만 보기 때문입니다(`timing.js`).
의미로 따지면 **블록의 마지막 문장이 `return`/`throw` 면** 그 뒤는 조건이 거짓일 때만
실행되므로, 문장 개수는 상관이 없습니다. 앞의 문장들은 나가는 길에 하는 일일 뿐입니다.

**어디에 걸리는가 (한 함수가 두 곳을 먹임 — 이게 핵심)**
`isEarlyReturnGuard()` 는 **두 자리에서** 쓰입니다:
1. `visitFunctionBody()` — 뒤따르는 setter 의 **`guarded`** → **언마운트 위험 판정**
2. `findGates()` — **관문 스텝**(보여주기)

→ 넓히면 관문 알약만 느는 게 아니라 **위험 경고가 조용히 사라지는 쪽**으로 움직입니다
(`guarded` 가 늘면 `unguardedDeferred` 가 줄고 `risk` 가 `null` 이 됨).
**이 프로젝트에서 더 무서운 방향**이므로, 넓히기 **전에** 위험 판정이 걸린
기존 테스트(`test/timing.test.mjs` 의 위험 판정 절)부터 확인할 것.

**같이 고쳐야 하는 것 — `stopKind()`**
지금은 블록의 **첫 문장**을 봅니다(`c.body[0]`). 문장이 둘 이상이 되면
`if (x) { log(); throw e }` 를 **"오류" 가 아니라 "중단"** 으로 잘못 적습니다.
→ **마지막 문장**을 보도록 함께 고칠 것.

**착수 순서 (권장)**
1. `isEarlyReturnGuard()` 를 "**else 없음 + 블록의 마지막 문장이 return/throw**" 로 넓힘.
2. `stopKind()` 를 마지막 문장 기준으로 고침.
3. `npm test` — **여기서 깨지는 테스트가 있으면 그게 판정 변화의 신호**. 숫자만 맞추지 말고
   무엇이 왜 바뀌었는지 보고 판단할 것.
4. 새 테스트: 잡아야 하는 것(`{ reset(); return }`) + **잡으면 안 되는 것**
   (else 붙은 if · await 뒤 · 나중에 불릴 콜백 안 — 기존 3개가 그대로 통과하는지)
   + `{ log(); throw e }` 가 **오류**로 적히는지.
5. 브라우저 실제 렌더 확인 → 커밋 → 이 문서 갱신 후 커밋.

**판단이 필요할 수 있는 곳**: 블록 안에서 **setter 를 부르고 나가는** 경우
(`if (x) { setY(); return }`). setY 는 이미 `조건부` 로 표시되고, 관문도 함께 서게 됩니다.
한 조건이 알약 두 개로 보이는 게 시끄러우면 그때 정할 것 — 미리 손대지 말 것.
