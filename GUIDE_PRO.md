...
`node_modules` 를 통째로 묶으면 `prismjs`(하이라이터가 **정적** import)가 같은 청크에
들어가고, 그 순간 `lib` 이 초기 로드의 정적 의존이 되어 **babel 300KB 가 첫 화면에
딸려옵니다.** 청크 규칙을 만질 땐 빌드 후 `dist/index.html` 에 `lib` 의
`modulepreload` 가 붙지 않는지 반드시 확인하세요.
...
파싱이 실패하면 `parseCode` 는 **실패 사유를 결과에 실어** 돌려줍니다.

```js
// ParseResult
{ …, totalLines, error: { message: string, line: number|null } | null }
```

`error` 가 있으면 **나머지 필드는 "세어 보니 없다" 가 아니라 "못 셌다"** 는 뜻입니다.
렌더러는 0 을 사실처럼 보여 주지 말고 이 필드를 먼저 봐야 합니다.

| 탭 | 잘린 코드에서 실제로 보이는 것 |
| :--- | :--- |
| ① 하이라이트 | **안내 카드 + 코드 자체** (섹션 밴드는 없음 — 어디가 잘렸는지 볼 수 있게 코드는 그림) |
| ② 구조맵 / ③ 메트릭 / ④ 플로우 | **안내 카드** (빈 화면·`0` 대신) |
| ⑤ 동작 | **안내 카드** (오류 위치 포함) |
| 상태바 | `함수: —` 처럼 **줄표** — `0` 과 구분됩니다 |
...
**오류 문장은 한 곳에서 만듭니다** (`src/core/parse-error.js`) — 두 분석기가 같은
`@babel/parser` 를 쓰므로 같은 오류를 만나는데, 서로 다른 말을 하면 사용자가 헷갈립니다.
테스트로 두 분석기의 메시지가 같은지 고정해 두었습니다.
